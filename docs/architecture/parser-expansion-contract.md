...
for parser-owned unsupported constructs. It does not add native glob expansion,
heredoc body collection, job control, programmable completion, full script mode,
or broad shell compatibility.
...
| Heredoc and here-string execution | Issue #10 |
...
| `pysh.parsing.multiline` | Quote continuation, backslash-newline joining, Python block coalescing, paste splitting, heredoc placeholder state. |
...
3. `pysh.parsing.lexer.strip_comments()` removes unquoted comments.
4. `pysh.parsing.grammar.validate_unsupported_syntax()` rejects parser-owned
   unsupported constructs with `UnsupportedSyntaxError`.
5. `pysh.parsing.expansion.expand_command_substitution()` expands `$(...)` and
   backtick substitution before command-chain execution.
6. `pysh.parsing.grammar.split_chain()` splits unquoted `;`, `&&`, and `||`.
7. Each chain element is split by `split_pipeline()` on unquoted `|`.
8. Each pipeline stage is tokenized with `shlex`, redirection is parsed, aliases
   are applied to the first word, and variable expansion runs on argv tokens.
9. `pysh.core.shell.PyShell` performs builtin dispatch, external command
...
| `*.py`, `?`, `[...]`, `**` | Passed literally; native expansion belongs to Issue #9. |
...
| Heredoc opener | Recognized only as unsupported syntax owned by Issue #10. |
